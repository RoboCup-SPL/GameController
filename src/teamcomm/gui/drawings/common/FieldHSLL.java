 ...
 */
public class FieldHSLL extends Static {

    @Override
    protected void init(GL2 gl) {
        RoSi2Loader.getInstance().cacheModels(gl, new String[]{"fieldHSLL"});
    }

    @Override
    public void draw(final GL2 gl, final Camera camera) {
        gl.glDepthFunc(GL2.GL_LESS);
        gl.glCallList(RoSi2Loader.getInstance().loadModel(gl, "fieldHSLL"));
